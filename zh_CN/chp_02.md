...
`Beta-Binomial 模型`的后验预测分布。在第一幅图中，实线曲线是小于或等于观测数据的预测值比率的核密度估计。虚线表示与观测数据大小相同的数据集的预期分布。在第二个子图上，黑线是一个核密度估计，表示预测值的比率小于或等于每次观测计算的观测值，而不是像第一个子图中的每个模拟。白线代表理想情况，标准均匀分布，以及期望在相同大小数据集上看到的该均匀分布的灰色偏差条带。
```

正如之前所说，我们有许多可供选择的 $T$ 统计量，用来总结观测和预测。

{numref}`fig:posterior_predictive_check_tstat` 显示了其中两个例子，第一个子图中的 $T$ 是均值，第二个子图中的 $T$ 是标准差。曲线是核密度估计，表示来自后验预测分布的 $T$ 统计量的分布，点是观测数据的值。

```{figure} figures/posterior_predictive_check_tstat.png
:name: fig:posterior_predictive_check_tstat
:width: 8.00in

`Beta-Binomial 模型` 的后验预测分布。在第一幅图中，实线曲线是平均值小于或等于观测数据的预测值的模拟比率的核密度估计。第二个子图与之相同，但描述的是标准差。黑点代表从观测数据计算的均值（第一组）或标准差（第二组）。
```

在继续阅读之前，你应该花点时间仔细检查 {numref}`fig:posterior_predictive_many_examples` ，并尝试理解为什么这些图看起来会这样。在这个图中，有一系列简单的例子来帮助我们获得关于解释后验预测检查图的直觉[^7]。在所有示例中，观测数据（蓝色）均遵循高斯分布。
