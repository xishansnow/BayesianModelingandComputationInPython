...
我们可以利用 **概率编程语言 Probabilistic Programming Languages** (PPL) 的帮助，而不是编写自己的采样器从头定义自己的模型。概率编程语言允许用户使用代码表达贝叶斯模型，然后借助通用推断引擎以相当自动化的方式执行贝叶斯推断。简而言之，概率编程语言帮助贝叶斯实践者更多地关注模型构建本身，而不是数学和计算细节。
