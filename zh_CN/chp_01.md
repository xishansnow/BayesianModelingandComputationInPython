...
在此情况下，`Jeffreys 先验` 能够保证：如果 `Alice` 和 `Bob` 都使用了 `Jeffreys 先验`，那么无论 `Bob` 做出哪种选择，都会得到相同的结果。也就是说，最终结果相对于所选择的参数具有不变性。此解释的一个推论是，除非使用 `Jeffreys 先验`，否则无法保证模型的两种或多种参数化必然会导致一致的后验。
